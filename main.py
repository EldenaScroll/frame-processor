...
class ProcessReq(BaseModel):
    key: str | None = None      # upload key (not used yet for this milestone)
    lot_id: str = "1"           # default lot

def cf_query(sql: str, params: list):
    """Call your Worker /query endpoint that talks to D1."""
    resp = requests.post(
        f"{CF_BASE_URL}/query",
...
def process(req: ProcessReq, request: Request):
    # Optional protection (recommended if you deploy Cloud Run with allow-unauthenticated)
    if PROCESSOR_SECRET:
    ...
    # 1) Ensure Space_1 row exists (your schema: space(id TEXT, lot_id TEXT, category TEXT, status INTEGER))
    exists = cf_query(
